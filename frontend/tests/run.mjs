...
assert.doesNotMatch(style, /--miot-list-height:[^;]*720px/);
...
assert.match(playerBar, /class="mini-stop-control[^"]*"[\s\S]*\/player\/stop/);
assert.match(style, /@media \(max-width: 599px\)[\s\S]*\.mini-mode-control\s*\{\s*display: none/);
assert.match(style, /@media \(max-width: 599px\)[\s\S]*\.mini-stop-control\.sl-button\s*\{\s*display: inline-flex/);
assert.match(modePopup, /value: 'single'/);
assert.match(modePopup, /value: 'random'/);
assert.match(modePopup, /value: 'singlePlay'.*label: '单曲播放'.*icon: 'looks_one_outlined'/);
...
assert.match(style, /\.player-sleep-popup[^}]*width: min\(280px/);
...
assert.match(style, /\.fullscreen-mobile-pager\s*\{[^}]*scroll-snap-type: x mandatory/);
